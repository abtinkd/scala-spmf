...
public class ItemsetHashTree {
        ...
        for (int i = 0; i < itemset.length; i++) {
            ...
            if (count == itemsetSize) {
                ...
                while (first <= last) {
                    ...
                    if (sameAs(list.get(middle), itemset, posRemoved) < 0) {
                        first = middle + 1;  //  the itemset compared is larger than the subset according to the lexical order
                    } else if (sameAs(list.get(middle), itemset, posRemoved) > 0) {
    ...
    /**
     * Check if two itemsets are the same
     *
     * @param itemset    the itemset
     * @param candidate  another itemset
     * @param posRemoved the position of an item that should be ignored in the second itemset
     * @return
     */
