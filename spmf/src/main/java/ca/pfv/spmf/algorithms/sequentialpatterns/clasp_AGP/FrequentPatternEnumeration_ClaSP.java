 ...
 * We can use different kind of IdList although we only makes a implementation:
...
public class FrequentPatternEnumeration_ClaSP {
     ...
     */
    public FrequentPatternEnumeration_ClaSP(AbstractionCreator abstractionCreator, double minSupport, Saver saver, boolean findClosedPatterns, boolean executePruningMethods) {
    ...
    /**
     * Execution of the search of frequent patterns by depth first search.
     *
     * @param verbose Flag for debugging purposes
     * @param patron  the pattern
     * @param trie    the trie
     */
    public void dfsPruning(Pattern patron, Trie trie, boolean verbose) {
        ...
        for (int i = 0; i < tam; i++) {
             ...
             */
            exploreChildren(new Pattern(eq.getPair()), eq, trie.getNodes(), trie.getNodes(), i + 1);
        }
    }

    private void exploreChildren(Pattern pattern, TrieNode currentNode, List<TrieNode> sequenceExtensions, List<TrieNode> itemsetsExtensions, int beginning) {
        ...
        for (TrieNode node : sequenceExtensions) {
             ...
             */
            IDList newIdList = currentTrie.getIdList().join(node.getChild().getIdList(), false, (int) minSupport);
        ...
        for (int i = 0; i < sequenceExtensionSize; i++) {
             ...
             */
            exploreChildren(newPattern, nodeToExtend, new_sequenceExtension, new_sequenceExtension, i + 1);
        }
        ...
        for (int k = beginning; k < itemsetsExtensions.size(); k++) {
             ...
             * order to know the appearances of the new pattern and its support.
             */
            IDList newIdList = currentTrie.getIdList().join(eq.getChild().getIdList(), true, (int) minSupport);
            //If the new pattern is frequent
            if (newIdList.getSupport() >= minSupport) {
                //We create a new trie for it
                Trie newTrie = new Trie(null, newIdList);
        ...
        for (int i = 0; i < itemsetExtensionSize; i++) {
             ...
             * make the i-extensions, but beginning from the (i+1)-th element
             */
            exploreChildren(newPattern, nodeToExtend, new_sequenceExtension, new_itemsetExtension, i + 1);
     ...
     * The hash keys used can vary, and we give some aproaches by the methods:
     * <p/>
     * key_standard()
    ...
    /**
     * One of the methods used by key2 in the method isAvoidable
    ...
    /**
     * One of the methods used by key2 in the method isAvoidable
     * that return the addition of the number of elements that appear
     * after of each appearance of the pattern in the sequences and sum
     * of the sequence identifiers where the given prefix appears
     *
     * @param idList IdList of the prefix to consider
     * @param trie   Trie of the pattern to consider
     * @return
