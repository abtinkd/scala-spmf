...
public class AlgoCloSpan {
    ...
    protected double relativeMinSupport;
    /**
    ...
    /**
     * Standard constructor. It takes the minimum support threshold (from 1 up
     * to 0) and an abstraction creator
     *
     * @param support            the minimum absolute support threshold
     ...
     */
    public AlgoCloSpan(double support, AbstractionCreator creator, boolean findClosedPatterns, boolean executePruningMethods) {
        this.minSup = support;
    ...
    public void runAlgorithm(SequenceDatabase database, boolean keepPatterns, boolean verbose, String outputFilePath) throws IOException {
        ...
        relativeMinSupport = (int) (database.size() * minSup);
        if (this.relativeMinSupport == 0) { // protection
            this.relativeMinSupport = 1;
        }
    ...
    protected void cloSpan(SequenceDatabase database, boolean keepPatterns, boolean verbose, boolean findClosedPatterns, boolean executePruningMethods, String outputFilePath) throws IOException {
         ...
         * database
         */
        PseudoSequenceDatabase pseudoDatabase = projectInitialDatabase(database, mapSequenceID, (int) relativeMinSupport);

        //We initialize the class that is in charge of managing the main loop of PrefixSpan
        RecursionCloSpan algorithm = new RecursionCloSpan(abstractionCreator, saver, (int) relativeMinSupport, pseudoDatabase, mapSequenceID, findClosedPatterns, executePruningMethods);
     ...
     */
    public double getRelativeMinSupport() {
        return relativeMinSupport;
    }
     ...
     * @return
     */
    private PseudoSequenceDatabase projectInitialDatabase(SequenceDatabase database, Map<Item, BitSet> mapSequenceID, long minSupport) {
        ...
        for (Sequence sequence : database.getSequences()) {
            ...
            Sequence optimizedSequence = sequence.cloneSequenceMinusItems(mapSequenceID, minSupport);
            if (optimizedSequence.size() != 0) {
                 ...
                 * in order not to affect to the minimum relative support
                 */
