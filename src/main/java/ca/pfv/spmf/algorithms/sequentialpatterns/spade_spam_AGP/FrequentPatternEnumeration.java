...
/**
 * This is an implementation of the main methods of SPADE algorithm. We keep open
 * the decision of which IdList to use. We have implemented three different kinds
 * of IdList so far:
 * 1) One based on hash map with arraylist (IDListStandard_Map)
 * 2) One based on hash map with bitsets (IDListBitMap)
 * 3) One based on a big bitmap with all the information kept inside (IDListFatBitmap)
 * <p/>
...
public class FrequentPatternEnumeration {
     ...
     *                           the case
     */
    public FrequentPatternEnumeration(CandidateGenerator candidateGenerator, double minSupport, Saver saver) {
     ...
     * @param dfs          Flag indicating if we are interested in a depth-first search if
     *                     activated. Otherwise, we understand that we are interested in a breadth-first
     *                     search
     * @param keepPatterns Flag to indicate if we want to keep the patterns found.
     * @param verbose      Flag for debugging purposes
     */
    public void execute(EquivalenceClass eq, boolean dfs, boolean keepPatterns, boolean verbose) {
        /*eq.setIdList(null);
        ...
        for (int i = eqMembers.size() - 1; i >= 0; i--) {
            ...
            for (int j = i; j >= 0; j--) {
                 ...
                 */
                List<Pattern> extensions = candidateGenerator.generateCandidates(child_X.getClassIdentifier(), child_Y.getClassIdentifier(), (int) minSupport);
                //For each candidate generated
                for (Pattern extension : extensions) {
                    /* Se make a join operation with the idlist of the equivalence
                     * classes that created the candidate
                     */
                    IDList newIdList = candidateGenerator.join(extension, child_X, child_Y, (int) minSupport);
                    //If the the pattern is frequent
                    if (newIdList != null && newIdList.getSupport() >= minSupport) {
            ...
            if (dfs) {
                ...
                if (anyPatternCreated) {
                    execute(child_X, dfs, keepPatterns, verbose);
                }
            }
        }
        ...
        if (!dfs) {
            ...
            if (anyPatternCreated) {
                for (int i = eqMembers.size() - 1; i >= 0; i--) {
                    ...
                    execute(eqMembers.get(i), dfs, keepPatterns, verbose);
    ...
    /**
     * Method to insert a class in their corresponding father. This father is that
     * is a prefix for the class given as a parameter.
     *
     * @param eq   Equivalence class with we want to insert as a member of its father
