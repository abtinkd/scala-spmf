...
class RecursionPrefixSpan_AGP {
     ...
     */
    public RecursionPrefixSpan_AGP(AbstractionCreator abstractionCreator, Saver saver, long minSupport, PseudoSequenceDatabase pseudoDatabase, Map<Item, BitSet> mapSequenceID) {
    ...
    public void execute(boolean keepPatterns, boolean verbose) {
        ...
        for (Item item : keySetList) {
            ...
            if (projectedContext != null && projectedContext.size() >= minSupport) {
    ...
    private void prefixSpanLoop(Pattern prefix, int k, PseudoSequenceDatabase context, boolean keepPatterns, boolean verbose) {
        ...
        for (Pair pair : pairs) {
            ...
            if (pair.getSupport() >= minSupport) {
                ...
                numberOfFrequentPatterns++;
                //If the projection exists and has more sequences than the minimum relative support
                if (projection != null && projection.size() >= minSupport) {
